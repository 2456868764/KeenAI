...
| P2-08 | Workflow 版本 draft→published + Trace | [x] `workflow_versions` publish snapshot、versions/rollback、unpublish/duplicate/archive、dry-run test API 已落地 |
| P2-09 | SLA 50/80/100% + Office Hours | [x] |
| P2-10 | Workflow Builder 多层 + canvas 内联配置 | [x] |
| P2-11 | Multimodal MM-01～07（[14-MULTIMODAL.md](./14-MULTIMODAL.md)） | [x] |
| P2-12 | Feedback schema + Board/Vote/Comment + AI 去重 | [x] |
...
| P3-04 | Multimodal MM-10～15 Agent 完整版 | [x] STT、image/video thumbnail、lite vision summary/extract_text、混合附件 plainText/FTS、Copilot native/text vision route、TTS、生图、IM 多模态均已落地 |
...
| I119-02 | CI 全绿 · `pnpm kb:bench` P95 记录 | [~] 本地 lint/typecheck/test/eval/bench、`core:parity:report`、`phase:gate:report` 已有；远程 CI artifact 和 deployed API bench 仍需外部证据 |
...
| MM-11 | image/video thumbnail + lite vision/extract_text enrichment + 混合附件 plainText/FTS + Inbox/Widget video/file render | [x] |
