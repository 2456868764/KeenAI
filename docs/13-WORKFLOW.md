...
当前实现：`@keenai/workflow` 的 `WORKFLOW_TRIGGERS` / `WorkflowDefinition` 已支持以上 16 类 trigger；运行时已落地 `page_view`、`new_messenger_conversation`、`first_message`、`any_message`、`teammate_message`、`teammate_added_note`、`conversation_state_changed`、`assigned_to_member`、`assigned_to_team`、`customer_unresponsive`、`teammate_unresponsive`、`ticket_created`、`ticket_state_changed`、登录态 `POST /api/v1/workflows/webhooks/trigger` 触发的 `webhook` workflow、登录态 `POST /api/v1/workflows/events/trigger` 按 `eventName` 精确匹配触发的 `event_match` workflow，以及 sync interval / Inngest cron / 登录态 `POST /api/v1/workflows/jobs/scan-scheduled` 扫描派发的 `schedule` workflow。
...
| 集成 | `script` ✅ | 同步 | 默认禁用；`WORKFLOW_SCRIPT_ENABLED=true` 后在 Node `vm` 中跑同步 JS（timeout 限制） |
| 集成 | `webhook_emit` ✅ | 同步 | 出站 Webhook |
| 集成 | `mcp_call` ✅ | 同步 | 调 MCP server tool（详见 09-AGENT-ENGINE.md） |

当前实现：`@keenai/workflow` 的 `WORKFLOW_BLOCK_TYPES` 与执行器已落地 `send_message`、`show_expected_reply_time`、`assign`、`close`、`reopen`、`end`、`goto`、`let_keeni_answer`、`wait`、`http_request`、`webhook_emit`、`mcp_call`、`script`、`branches`、`apply_rules`、`apply_sla`、`convert_to_ticket`、`link_ticket`、`send_ticket_form`、`send_ticket_update`、`set_ticket_state`、`collect_data`、`collect_customer_reply`、`reply_buttons`、`disable_customer_reply`、`snooze`、`csat`、`tag_end_user`、`tag_conversation`、`add_note`、`mark_priority`。其中 `collect_data` / `send_ticket_form` / `collect_customer_reply` / `reply_buttons` / `csat(waitForRating)` 支持挂起恢复，`collect_customer_reply` 会可选发送提示消息，默认等待 `conversation/message.received` 后 2 秒缓冲再恢复，并在 step output 记录 `customerReplyMessageId` / `customerReplyText`，`send_ticket_form` 会发送客户可见 ticket form，优先写入指定或当前 conversation 关联 ticket，没有关联 ticket 时自动从 conversation 创建 ticket，Widget 提交后验证字段类型并写入 `tickets.customFields` 与 `workflow_ticket_form_submitted` event，`assign` 支持直接分配个人/团队，并支持基于团队成员的 `round_robin` 与 `least_busy` 策略写入最终 `assigneeId` / `teamId`，`tag_end_user` 会按当前 conversation 的 `userId` 将 `workflowEndUserTags` 同步写入同 org 下该 end user 的所有 conversations，`end` 会显式终止当前 linear path，`goto` 会跳转到已存在的目标 block 且由运行时 cycle detection 阻断循环，`add_note` 会写入 `agent + isInternal + sentVia=workflow` 的内部消息且不会反向触发 `teammate_added_note` workflow，`mark_priority` 会更新 conversation priority，`reopen` 会将 conversation 恢复为 open 并清理 `closedAt` / `snoozedUntil`，`set_ticket_state` 会按 `ticketId` 或当前 conversation 关联 ticket 解析目标并复用 ticket status transition，`apply_sla` 会按指定或当前启用 SLA policy 评估当前 conversation 并记录 breach events，`show_expected_reply_time` 会基于指定或当前启用 SLA policy 的 `firstResponseSec`、Office Hours 状态和 fallback minutes 写入客户可见的 workflow 预计回复时间消息，`disable_customer_reply` 会写入 conversation attributes 并由 Widget API 阻断后续客户消息，`webhook_emit` 会向外部 URL POST 标准 JSON envelope（event / payload / workflow context）并记录 HTTP 状态，`mcp_call` 会通过 API 共享 MCP Host 按 `serverId + toolName + arguments` 调用外部 stdio MCP tool 并在 step output 记录结果预览，`script` 会在 `WORKFLOW_SCRIPT_ENABLED=true` 时以 Node `vm` 执行同步 JS 函数体，并将 JSON-serializable 返回值写入 step output；默认关闭时报 `workflow_script_disabled`。
...
- **Collect / Ticket forms**：字段 key / label / required / type / options / auto-close 均在节点内编辑，字段上限与 DSL 保持一致（Collect Data 8 个，Ticket Form 16 个）
- **Branches/Apply rules**：节点底部 `+ Add branch` 弹出 Predicate 编辑器（Field selector + Op + Value）；Branches / Apply Rules 均可在节点内编辑完整 16 条 DSL 上限
...
|----|:---:|----|
| 13 种 Trigger | ✅ | § 3 |
| 全部 Action Block | 🚧 | § 4（当前已落地 21 类，剩余逐项补齐） |
