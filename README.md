...
|------|------------------|------|
| **Inbox** 统一收件箱 | Support Inbox | 🚧 规划中 |
| **Channels** 全渠道接入 | Messenger + Email | 🚧 规划中 |
| **Conversations** 对话生命周期 | Conversations | 🚧 规划中 |
| **Tickets** 结构化工单 | Customer/Back-office/Tracker | 🚧 规划中 |
| **Workflows** 无代码自动化 | Workflows | 🚧 规划中 |
| **Keeni AI Agent** 客服 AI | Fibi AI Agent | 🚧 规划中 |
| **Copilot** 客服辅助 AI | Copilot | 🚧 规划中 |
...
### 5.1 Lite 模式（默认，单容器 + SQLite）

> Indie / 自托管单机 / Demo —— **零外部依赖**，Bun 单二进制 + 一个 `.db` 文件，含向量检索 + 全文检索 + AI Agent。
...
# 数据：     ./data/keenai.db（SQLite + sqlite-vec + FTS5 + 附件）
# 备份：     可选 litestream sidecar 实时增量到 S3
```

或直接用 `bun create`（无 Docker）：

```bash
bun create keenai my-keenai
cd my-keenai
bun install
bun run db:migrate
bun run dev
...
### 5.2 Standard 模式（PostgreSQL + Redis + Meilisearch + MinIO）
...
### 5.3 Cloud 模式（Vercel / Cloudflare + 托管 PG）
